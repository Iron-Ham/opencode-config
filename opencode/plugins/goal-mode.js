  ...
  return {
    ...
    tool: {
      ...
      update_goal_objective: {
        ...
        async execute(args, context) {
          const input = args;
          const requested = toolOptions(input).status ?? "active";
      ...
      update_goal: {
        ...
        async execute(args, context) {
          ...
          if (input.status === "complete") {
            const goal2 = await completeGoal(context.sessionID, details.evidence ?? "");
          ...
          const goal = await markGoalUnmet(context.sessionID, details.blocker ?? "");
