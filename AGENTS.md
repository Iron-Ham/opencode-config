...
- Use `build` as the GPT-5.6 Terra xhigh controller and owner of durable production implementation. Use `general` for a substantial independent writable slice; it inherits the invoking Build or Ultra model unless the developer explicitly overrides it. Use Terra-backed `plan` for read-only planning, the Luna primary for bounded cost-sensitive implementation with strong deterministic checks, the Sonnet or Sol primary when that model should implement, and `explore` for bounded repository discovery. Use inline read/search tools for trivial lookups. Machine-local routing may replace role-based models; model-branded primaries remain pinned, but personal Luna, Terra, Sonnet, and Sol slash commands are not installed. Open-weight experiment commands pin both model and provider.
