...
The managed top-level `external_directory` permission allows the active workspace's controllers and subagents to inspect paths outside that workspace. It is intentionally shared rather than copied into individual agent definitions, so delegated work uses the same filesystem scope as its controller. The shared policy denies common credential stores, including AWS, SSH, Kubernetes, Docker, GnuPG, system-config, and macOS Keychain paths. Tool-specific safeguards remain in force: all managed agents deny `.env`-style reads, read-only specialists cannot edit or execute commands, and `general` retains its destructive-command denials.
